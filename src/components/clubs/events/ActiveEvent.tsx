...
export function ActiveEvent() {
  ...
  let formattedDate;
  if (event) {
    formattedDate = event?.eventInfo?.start_time ? new Date(event.eventInfo.start_time).toDateString() : '';
  }

  return (
    event?.clubInfo && event.eventInfo ? (
      <Card className="flex w-full overflow-hidden items-center justify-center">
        <CardContent className="px-10">
          ...
          <CardFooter className="justify-start flex flex-col items-start pt-5">
            <h4 className="pl-5 font-sans text-lg font-semibold tracking-normal">Responses</h4>
            {event && <EventResponses event_id={event?.eventInfo.id} />}
          </CardFooter>
        </CardContent>
      </Card>
    ) : (
